...
export const App = () => {
  ...
  return (
    <div className={`screen scanlines`}>
      {/*<img alt="background-step-1" width={'100%'} height={'100%'} src={ImageUrl} />*/}
      <div ref={htmlPortalRef} id="html-portal"></div>
      <canvas ref={canvasPortalRef} id="canvas-portal" />

      <Canvas shadows dpr={[1, 1]} camera={{ position: [0, 2, 9.5], fov: 50 }} resize={{ scroll: true, debounce: { scroll: 50, resize: 50 } }}>
        <Suspense fallback={null}>
          <>
            {/*<color attach="background" args={['#0d151d']} />*/}
            <MainScene debugMode={debugMode} canvasPortalRef={canvasPortalRef} htmlPortalRef={htmlPortalRef} />
            {debugMode ? (
              <>
