...
var fract = (function () {
  ...
  module.init = function (config) {
    c = document.getElementById("canvas-glyph");
    ...
    fract.segments = makeThumb({
      min: 0,
      steps: config.segments,
      step: config.segments,
    });
    fract.mirror = makeThumb({
      min: 0,
      steps: config.mirror,
      step: config.mirror,
    });
    ...
    fract.angle.fix = function () {
      ...
      if (Math.abs(skew) > fract.skewangle.steps / 2) {
        fract.skewangle.step =
          (fract.skewangle.step + fract.skewangle.steps / 2) %
          fract.skewangle.steps;
      } else {
  ...
  module.auto = (function () {
    ...
    var smoothstart = function () {
      ...
      if (
        fract.color.bg === fract.color.white &&
  ...
  module.al = (function () {
    ...
    var prepare = function prepare() {
      ...
      var skewrad =
        (fract.angle.step - fract.skewangle.step + fract.skewangle.steps / 2) *
        5 *
        (Math.PI / 180);
      ...
      if (motifWidth > 0.98) {
        ...
        unit = Math.abs(
          width / zoom / (motifWidth - Math.cos(skewCorrection) * 2)
        );
  ...
  var makeSlider = function (spec) {
    ...
    outward.resize = function () {
      outward.range =
        outward.element.parentNode.clientWidth - outward.element.clientWidth;
    ...
    outward.random = function () {
      outward.update(
        Math.floor(
          Math.random() * (outward.steps - outward.min + 1) + outward.min
        )
      );
    };
    ...
    var getx = function (mpos) {
      ...
      if (mpos < outward.start + outward.element.clientWidth / 2) {
        ...
        mpos >
        outward.start + outward.range + outward.element.clientWidth / 2
      ) {
    ...
    outward.step = spec.step;
    outward.element.style.left =
      outward.step * (outward.range / outward.steps) + "px";
  ...
  var makeThumb = function (spec) {
    ...
    renders.poly = function (step) {
      var length = (c.width * 2) / 3,
        polyOffset =
          length / 2 / Math.tan((180 / (step + 1)) * (Math.PI / 180)),
        x,
        y,
